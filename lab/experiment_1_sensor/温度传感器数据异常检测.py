...
返回：
outliers: 异常值的索引列表、mean、data数组大小、标准差w
"""
...
class MyGUI(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('专业实验一：温度传感器异常值检测')     # 界面标题
        self.setGeometry(300, 300, 800, 800)    # 界面大小设置
        ...
        self.btn_detect.clicked.connect(self.detect_outliers_and_plot)  # connect函数,连接到数据监测与绘图函数
        ...
        self.btn_retest.clicked.connect(self.reset_data_and_redetect_outliers)  # connect函数,连接到reset函数
        ...
        self.btn_exit.clicked.connect(self.close)   # connect函数,连接到close函数
    ...
    """
    def generate_simulated_data(self, num_samples=500, num_errors=3):
        seed = np.random.randint(1000000)  # Generate a new seed each time
        np.random.seed(seed)
        data = np.random.normal(loc=25, scale=1, size=num_samples)  # 生成正态分布的正常测量值序列
        error_indices = np.random.choice(num_samples, num_errors, replace=False)    # 生成随机的异常值索引
        error_values = np.random.normal(loc=25, scale=15, size=num_errors)  # 生成异常值25±15（可以调节范围）
        data_with_errors = np.copy(data)    # 已生成的包含异常值的数组
